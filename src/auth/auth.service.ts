...
export class AuthService {
  ...
  async verifyUser(id: string) {
    try {
      const verifiedStatusResponse = await this.userRepository
      ...
      if (verifiedStatusResponse.affected) {
        return {
          statusCode: 201,
    ...
    try {
      ...
      try {
        ...
        throw new InternalServerErrorException(
          'Something went wrong : Password is not updated',
        );
      }
    } catch (error) {
      throw new InternalServerErrorException(
  ...
  async metamaskAddressUpdate(updateMetamaskAddress: UpdateMetamaskAddressDto) {
